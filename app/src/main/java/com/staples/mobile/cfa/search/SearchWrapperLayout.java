...
public class SearchWrapperLayout extends LinearLayout {
    ...
    public SearchWrapperLayout(Context context) {
        super(context, null, 0);
    }

    public SearchWrapperLayout(Context context, AttributeSet attrs) {
        super(context, attrs, 0);
    }

    public SearchWrapperLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void onMeasure(int widthSpec, int heightSpec) {
        ...
        // handling for GridView
        View gridViewLayout = getChildAt(0);
        if (gridViewLayout instanceof GridView) {
            GridView grid = (GridView) gridViewLayout;
            ...
            // handling for SearchAdapter
            ListAdapter adapter = grid.getAdapter();
            if (adapter instanceof SearchAdapter) {
                SearchAdapter searchAdapter = (SearchAdapter) adapter;
                if (n > 1) {
                    searchAdapter.setLayout(R.layout.search_result_row_tall);
                }
                else {
                    searchAdapter.setLayout(R.layout.search_result_row_wide);
                }
            }
        }
    }
